...
const VenueTemplate = ({ props, i, router }: _Object) => {
  const dispatch = useDispatch<AppDispatch>()
  const basePath = router.query.types==='caterers' ? '/caterers' : '/venues';
  ...
  return (
    <div className="card">
      <div className="img-wrapper">
        <Link href={`${basePath}/${props.slug}`} className="">
      ...
      <div className="card-body">
        ...
        <h4 className="venue-name">
          <Link href={`${basePath}/${props.slug}`}>{props?.title || ''}</Link>
        </h4>
        ...
        <div className={`footer d-flex justify-content-between ${props?.extraOptions?.paxPrice > 0 ? '' : 'venus-button-center'}`}>
          ...
          <Link href={`${basePath}/${props.slug}`} passHref legacyBehavior className="btn btn-primary">
            <a className="btn btn-primary" rel="noopener noreferrer">{router.query.types==='caterers' ? 'View Caterers' : 'View Venue'}</a>
          </Link>
        </div>
      </div>
    </div>
