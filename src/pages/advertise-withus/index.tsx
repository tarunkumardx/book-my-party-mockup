...
const ContactUs = (props: _Object) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: _Object) => {
      setLoading(true)
      const result = await bookingService.create(values, 13)
      if (result.is_valid) {
        setLoading(false)
  ...
  return (
    <>
      <SEOHead seo={{
        title: 'Advertise With Us - Book My Party', metaDesc: 'Connect with us for unforgettable events! Whether it\'s birthdays, weddings, or corporate gatherings, our experts ensure seamless celebrations. Contact us today!'
      }} />
      <Layout {...props
      } >
