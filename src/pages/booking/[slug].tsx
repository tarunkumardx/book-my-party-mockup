...
const Booking = () => {
  ...
  useEffect(() => {
    if(router.query.types === 'farm-house'){
      setMinPax(1);
      setMaxPax(5000);
    }
    if(router.query.types === 'banquet'){
      setMinPax(1);
      setMaxPax(5000);
    }
    if(router.query.types === 'caterers'){
      setMinPax(1);
      setMaxPax(5000);
    }
    if(router.query.types === 'restaurant'){
      setMinPax(1);
      setMaxPax(200);
    }
    if(router.query.types === 'fun-zone'){
      setMinPax(1);
      setMaxPax(1000);
    }
  ...
  const minPaxValue = venueDetails?.extraOptions?.packages?.find((item: _Object) => item.title?.trim() === formik?.values?.input_31)?.minPax || 1
  useEffect(() => {
    if(minPaxValue > router?.query?.pax){
      setShowPaxModal(true);
    }},[minPaxValue, router?.query?.pax])
  return (
    <Layout {...venueDetails}>
      <div>
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="text-center p-20">
              <strong>Note:- </strong>You have selected a package that requires a minimum of {minPaxValue} Pax
            </div>
          </div>
        </div>
      </div >)}
                      ...
                      <div className="col-md-6 col-lg-3 col-xl-3">
                        <div className={`inner booking-inner ${isActive}`}>
                          ...
                          <Quantity
                            ...
                            decrementQuantity={() => {
                              if (formik?.values?.input_33 > minPaxValue) {
                                const newValue = Math.max(minPax-1, formik.values.input_33 - 1);
                                formik.setFieldValue('input_33', (newValue < minPax)? minPax: newValue);
                            ...
                            incrementQuantity={() => {
                              const newValue = Math.min(maxPax+1, formik.values.input_33 + 1);
                              formik.setFieldValue('input_33', (newValue > maxPax)? maxPax: newValue);
                              setIsActive(newValue > minPax);
                              if (newValue > maxPax) {
                                setMessage(`The number of guests cannot exceed ${maxPax}`);
                              } else {
                      ...
                      <div className="d-flex align-items-center">
                        ...
                        <InputField
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                            ...
                            if (!isFloat(value) && value >= minPaxValue) {
                              formik.setFieldValue('input_33', parseInt(e.target.value) <= 0 ? 1 : e.target.value);
                            } else {
                              alert(`Note:- You have selected a package that requires a minimum of ${minPaxValue} Pax`)
                            }
                          }}
